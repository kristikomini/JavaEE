...
class EnrollmentApiIT {
    ...
    void enrollEndToEnd() {
        ...
        // The Location header is an absolute path as the SERVER sees it, so
        // it carries the context path. Built from contextPath rather than
        // written out, so it survives a change to that setting.
        assertThat(response.getHeaders().getLocation().getPath())
                .matches(Pattern.quote(contextPath) + "/api/enrollments/\\d+");
    ...
    void withdrawalReleasesTheSeat() {
        ...
        // absolute(), not url(): `location` came from the Location header
        // and already begins with the context path. Passing it through
        // url() would prefix it a second time and 404.
        ResponseEntity<String> withdrawal = rest.postForEntity(
                absolute(location + "/withdrawal"), null, String.class);
    ...
    /** A path BELOW the context path, e.g. {@code "/api/courses"}. */
    private String url(String path) {
        return "http://localhost:" + port + contextPath + path;
    }

    /**
     * A path that ALREADY includes the context path - typically one taken
     * straight from a {@code Location} header, which the server writes as an
     * absolute path from the root.
     */
    private String absolute(String pathIncludingContext) {
        return "http://localhost:" + port + pathIncludingContext;
    }
